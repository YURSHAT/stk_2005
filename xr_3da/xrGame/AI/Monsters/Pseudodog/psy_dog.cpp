...
void CPsyDogPhantom::Think()
{
	...
	Level().Cameras().AddPPEffector(xr_new<CMonsterEffector>(m_appear_effector.ppi, m_appear_effector.time, m_appear_effector.time_attack, m_appear_effector.time_release));
}
...
{
	if (is_wait_to_destroy_object()) return;
	if ((who == EnemyMan.get_enemy())  && (who != 0)) destroy_me();
}
