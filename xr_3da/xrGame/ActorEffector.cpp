...
{
	SetType((ECamEffectorType)effAlcohol);
	SetCyclic(true);
	SetFactorFunc(GET_KOEFF_FUNC(c, &CActorCondition::GetAlcohol));
	Start			("camera_effects\\drunk.anm");
}

CFireHitCamEffector::CFireHitCamEffector(ECamEffectorType type, float power)
{
	SetType((ECamEffectorType)type);
	SetCyclic(false);
	SetFactorFunc(GET_KOEFF_FUNC(this, &CFireHitCamEffector::GetPower));
	m_power			= power;
