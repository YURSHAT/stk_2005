...
{ 
	...
	CHK_DX	(MultiByteToWideChar(CP_ACP, 0, filename, -1, filename_UNICODE, 4096 ));

	IGraphBuilder	*pGraph			;
	IMediaControl	*pMediaControl	;
	IMediaEvent		*pEvent			;
	IVideoWindow	*pVidWin		;
	CoInitialize	(NULL);			// Create the filter graph manager and query for interfaces. 
	CoCreateInstance(CLSID_FilterGraph, NULL, CLSCTX_INPROC_SERVER, IID_IGraphBuilder, (void **)&pGraph); 
	pGraph->QueryInterface		(IID_IMediaControl,	(void **)&pMediaControl	); 
	pGraph->QueryInterface		(IID_IMediaEvent,	(void **)&pEvent		);
	pGraph->RenderFile			(filename_UNICODE, NULL);				// Build the graph. 
	pGraph->QueryInterface		(IID_IVideoWindow, (void **)&pVidWin);	// Get Window
	pVidWin->put_FullScreenMode	(OATRUE);								// Switch to full-screen
...
int PASCAL IntroDSHOW_wnd	(HINSTANCE hInstC, HINSTANCE hInstP, LPSTR lpCmdLine, int nCmdShow)
{
	MSG msg={0};
	WNDCLASS wc;
	USES_CONVERSION;
